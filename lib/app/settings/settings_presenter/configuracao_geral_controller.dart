...
class ConfiguracaoGeralController extends GetxController {
  ...
  Future<SignOutStatus> singOut() async {
    return await signOutUsecase().then((value) {
      if (value == SignOutStatus.success) {
  ...
  Future<RecuperarSenhaEmailStatus> recuperarSenha(
      {@required String email}) async {
    return await recuperarSenhaEmailLoginUsecase(email: email);
  }

  Future<SignInStatus> signInGoogleLogin() async {
    singOut();
    SignInStatus result = await signInGoogleUsecase().then((value) async {
  ...
  Future<SignInStatus> signInEmailLogin({
  ...
  Future<SignInStatus> novoEmailLogin({
  ...
  _carregarUsuario() async {
    CarregarUsuarioStatus usuarioLogado = await carregarUsuario();
    if (usuarioLogado == CarregarUsuarioStatus.success) {
      this.usuarioFirebase = usuarioLogado.successGet;
    }
  }
  ...
  void _carregarEmpresa() async {
    CarregarEmpresaStatus result = await carregarEmpresa();
    if (result == CarregarEmpresaStatus.success) {
      this.empresaFirebase = result.successGet;
    }
  }
  ...
  void _getCon() async {
    ChecarConeccaoStatus teste = await checarConeccao();
    this.estaConectado = teste.successGet;
    _mostrarConeccao();
  }
  ...
  final _allSecao = List<FirebaseResultadoSecaoModel>().obs;
  get allSecao => this._allSecao;
  set allSecao(value) => this._allSecao.bindStream(value);
  ...
  Future<void> _getAllSecao() async {
    ...
    if (result == CarregarSecaoStatus.success) {
      this.allSecao = result.successGet;
    }
  }
  ...
  //Carregar Theme
  void _getSettingsTheme() async {
    CarregarThemeStatus result = await carregarTheme();
    if (result == CarregarThemeStatus.success) {
      this.fireTheme = result.successGet;
    }
  }
